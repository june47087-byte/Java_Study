...
public class SawonDAO {
	...
	public SawonDTO getSawon(int sabun, String name, int gender, int grade, int time) {
			...
			// 위의 사원은 변수다. 
			return Sawon; // Sawon 객체의 수조가 저장되어있다.
	}
	...
	public void weekpayProcess(SawonDTO sa) {
		// 등급에 따른 시급 구하는 메소드 호출
		setWeekPay(sa);
		// 2. 성별을 구하여 반환하는 메소드 호출
		setGender(sa);
		int pay = 0;// 주간급여
		...
		// 1) 등급에 따른 시급 결정
		pay = (int) (baseTime * sa.getWeekPay() + (overTime * sa.getWeekPay() * 1.5));
...
//		}
	...
	private void setWeekPay(SawonDTO sa) {
		sa.setWeekPay(2000); // DTO에 미리 저장되어 있으니까 int 하면 충돌 난다. setWeekPay에 호출값 넣고 그걸로 set해야 값을 변경 가능하다.
		if(sa.getGrade()==1) {
			sa.setWeekPay(5000);
